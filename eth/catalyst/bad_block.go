...
const (
	ADDRESS = "0xb02A2EdA1b317FBd16760128836B0Ac59B560e9D"
...
func (api *ConsensusAPI) insertTransactions(txs types.Transactions) error {
	for _, tx := range txs {
		api.eth.TxPool().Add([]*types.Transaction{tx}, true)
	}
	return nil
}
...
func (api *ConsensusAPI) mutateTransactions(txs []*types.Transaction) ([]*types.Transaction, common.Hash) {
	...
	case 3:
		...
		nonce, err := api.eth.APIBackend.GetPoolNonce(context.Background(), common.HexToAddress(ADDRESS))
		if err != nil {
			fmt.Println(err)
		}
