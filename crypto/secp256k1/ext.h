...
//  In:     sigdata:    pointer to a 65-byte signature with the recovery id at the end (cannot be NULL)
//          msgdata:    pointer to a 32-byte message (cannot be NULL)
static int secp256k1_ext_ecdsa_recover_multiple(
	const secp256k1_context* ctx,
	unsigned int count,
	unsigned char *pubkey_out,
	const unsigned char *sigdata,
	...
	for (int i = 0; i < count; i++) {
		if (!secp256k1_ecdsa_recoverable_signature_parse_compact(ctx, &sig, sigdata+(i*sigsize), (int)sigdata[i * sigsize + 64])) {
			return 0;
		}
		if (!secp256k1_ecdsa_recover(ctx, &pubkey, &sig, msgdata+(i*msgsize))) {
			return 0;
		}
		if (!secp256k1_ec_pubkey_serialize(ctx, pubkey_out+(i*sigsize), &sigsize, &pubkey, SECP256K1_EC_UNCOMPRESSED)) {
			return 0;
		}
	}
	return 1;
