...
func (miner *Miner) commitTransactions(env *environment, txs *transactionsByPriceAndNonce, interrupt *atomic.Int32, minTip *big.Int) error {
...
func (miner *Miner) fillTransactions(interrupt *atomic.Int32, env *environment) error {
	pending := miner.txpool.Pending(true)
	...
	if len(localTxs) > 0 {
		...
		if err := miner.commitTransactions(env, txs, interrupt, new(big.Int)); err != nil {
			return err
		}
	}
	if len(remoteTxs) > 0 {
		txs := newTransactionsByPriceAndNonce(env.signer, remoteTxs, env.header.BaseFee)
		if err := miner.commitTransactions(env, txs, interrupt, tip); err != nil {
			return err
		}
	}
	return nil
