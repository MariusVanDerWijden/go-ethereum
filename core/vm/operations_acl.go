...
func makeCallVariantGasCallEIP7702(oldCalculator gasFunc) gasFunc {
	return func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (GasCosts, error) {
		...
		if target, ok := types.ParseDelegation(evm.StateDB.GetCode(addr)); ok {
			...
			if !contract.UseGas(GasCosts{RegularGas: cost}, evm.Config.Tracer, tracing.GasChangeCallStorageColdAccess) {
				return GasCosts{}, ErrOutOfGas
			}
			total += cost
		}
		...
		// tracers.
		contract.Gas.RegularGas += total

		var overflow bool
		if total, overflow = math.SafeAdd(old.RegularGas, total); overflow {
			return GasCosts{}, ErrGasUintOverflow
		}
		return GasCosts{RegularGas: total}, nil
	}
}
